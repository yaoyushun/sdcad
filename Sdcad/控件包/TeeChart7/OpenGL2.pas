...
// GLU types:
type
      TGLUNurbs           = {$IFDEF CLR}TObject{$ELSE}record end{$ENDIF};
      TGLUQuadric         = {$IFDEF CLR}TObject{$ELSE}record end{$ENDIF};
      TGLUTesselator      = {$IFDEF CLR}TObject{$ELSE}record end{$ENDIF};
...
//------------------------------------------------------------------------------
    ...
    wglGetProcAddress : function(ProcName: {$IFDEF CLR}String{$ELSE}PChar{$ENDIF}): Pointer; stdcall;
    ...
    ChoosePixelFormat : function(DC: HDC; {$IFDEF CLR}var {$ENDIF} PFD: {$IFDEF CLR}TPixelFormatDescriptor{$ELSE}PPixelFormatDescriptor{$ENDIF}): Integer; {$IFNDEF CLR}stdcall;{$ENDIF}
//    DescribePixelFormat : function(DC: HDC; p2: Integer; p3: UINT; PFD: PPixelFormatDescriptor): Integer; stdcall;
    DescribePixelFormat: function(DC: HDC; p2: Integer; p3: UINT; var p4: TPixelFormatDescriptor): BOOL; {$IFNDEF CLR}stdcall;{$ENDIF}

    GetPixelFormat : function(DC: HDC): Integer; {$IFNDEF CLR}stdcall;{$ENDIF}
    SetPixelFormat : function(DC: HDC; p2: Integer; {$IFDEF CLR}const{$ENDIF} PFD: {$IFDEF CLR}TPixelFormatDescriptor{$ELSE}PPixelFormatDescriptor{$ENDIF}): BOOL; {$IFNDEF CLR}stdcall;{$ENDIF}
    SwapBuffers : function(DC: HDC): BOOL; {$IFDEF CLR}{$ELSE}stdcall;{$ENDIF}
...
procedure CloseOpenGL;
function InitOpenGL: Boolean;
function InitOpenGLFromLibrary(const GL_Name, GLU_Name: String): Boolean;
...
//  SwapBuffers:=GetProcAddress(GLHandle,'wglSwapBuffers');
  if not assigned(SwapBuffers) then
     SwapBuffers:=Windows.SwapBuffers;
...
//  GetPixelFormat:=GetProcAddress(GLHandle,'wglGetPixelFormat');
  if not assigned(GetPixelFormat) then
     GetPixelFormat:=Windows.GetPixelFormat;

//  SetPixelFormat:=GetProcAddress(GLHandle,'wglSetPixelFormat');
  if not assigned(SetPixelFormat) then
     SetPixelFormat:=Windows.SetPixelFormat;

//  DescribePixelFormat:=GetProcAddress(GLHandle,'wglDescribePixelFormat');
  if not assigned(DescribePixelFormat) then
     DescribePixelFormat:=Windows.DescribePixelFormat;
end;
...
begin
  ...
  tmp:=Pos(' ',Buffer);
  if tmp>0 then Delete(Buffer,tmp,Length(Buffer)-tmp+1);
...
{$IFNDEF CLR}
  ...
  begin
    ...
    nVersion:=1;
    dwFlags:=PFD_SUPPORT_OPENGL or PFD_SWAP_COPY;  { 5.01 }
    ...
    dwFlags:=dwFlags or PFD_GENERIC_ACCELERATED; // or PFD_GENERIC_FORMAT;
...
//     Raise Exception.Create('MakeCurrent '+IntToStr(GetLastError))
  ;
...
Function InternalFreeLibrary(hLibModule: HMODULE): BOOL;
begin
  { do not free library in Windows 95 (lock, bug) }
...
procedure CloseOpenGL;
begin
  if GLHandle > 0 then begin InternalFreeLibrary(GLHandle); GLHandle:=0; end;
  if GLUHandle > 0 then begin InternalFreeLibrary(GLUHandle); GLUHandle:=0; end;
  ClearProcAddresses;
  ClearExtensions;
  FirstContext:=True;
end;
...
begin
  ...
  begin
    ...
    if not Result then Result:=InitOpenGLFromLibrary('OpenGL.DLL','GLU.DLL');
  end
  else Result:=True;
end;
...
{$IFNDEF CLR}
  ...
  try
  {$ENDIF}
    GLHandle:={$IFDEF D5}SafeLoadLibrary{$ELSE}LoadLibrary{$ENDIF}(PChar(GL_Name));
    GLUHandle:={$IFDEF D5}SafeLoadLibrary{$ELSE}LoadLibrary{$ENDIF}(PChar(GLU_Name));
    ...
    begin
      if GLHandle > 0 then InternalFreeLibrary(GLHandle);
      if GLUHandle > 0 then InternalFreeLibrary(GLUHandle);
    end;
  {$IFNDEF D5}
  finally
