...
Uses {$IFDEF CLR}Borland.VCL.Classes{$ELSE}Classes{$ENDIF};
...
Const  { do not translate }
   ...
   AXVer = TeeChartVersion+TeeAXMinorVersion;
   {$ELSE}
   TeeVCLMinorVersion        ='';  // ie: '01'  '02' , etc
   {$ENDIF}

   TeeMsg_Version            ='TeeChart '+{$IFDEF TEELITE}'Lite'{$ELSE}'Pro'{$ENDIF}+' v'+TeeChartVersion+{$IFDEF TEEOCX}TeeAXMinorVersion{$ELSE}TeeVCLMinorVersion{$ENDIF};
   ...
   TeeMsg_TeeChartPalette    ={$IFDEF CLX}'TeeChart '{$ELSE}'TeeChart'{$ENDIF}{$IFDEF TEELITE}+' Lite'{$ENDIF};
...
Function TeeDefaultFilterExtension:String;
...
Function TeeDefaultFilterExtension:String;
begin
  result:='(*.'+TeeMsg_TeeExtension+')|*.'+TeeMsg_TeeExtension;
end;
