...
var tmpColor    : TColor;
   ...
   begin
     ...
     Begin
       ...
       if FAreaBrush.Color=clTeeColor then tmpColor2:=SeriesColor;
   ...
   begin
     ...
     begin
       ...
       Begin { line 2D }
         if FColorEachLine then LinePrepareCanvas(Canvas,tmpColor)
...
begin
  inherited;
  if not (Source is Self.ClassType) then FPointer.Hide;
  FDrawArea:= False;
  FDrawLine:= True;
end;
...
Begin
  ...
  begin
    tmpSumAbs:=0;
    for t:=FirstValueIndex to LastValueIndex do
        tmpSumAbs:=tmpSumAbs+Abs(YValues.Value[t]);
  end
  else
...
var tmp : Integer;
    t   : Integer;
begin
  if ParentChart.MaxPointsPerPage>0 then  // 7.0
  begin
