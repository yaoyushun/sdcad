...
Var TeeBulgarianLanguage:TStringList=nil;

Procedure TeeSetBulgarian;

implementation

Uses SysUtils, TeeConst, TeeProCo {$IFNDEF D5},TeCanvas{$ENDIF};

Procedure TeeBulgarianConstants;
begin
  { TeeConst }
  ...
  //TeeMsg_InvertedScroll       := '&Inverted Scroll';
  TeeMsg_Box		      := 'Box';
...
Procedure TeeSetBulgarian;
begin
  if not Assigned(TeeBulgarianLanguage) then
  begin
    TeeBulgarianLanguage:=TStringList.Create;
    TeeBulgarianLanguage.Text:=
...
'LABEL AND PERCENT TOTAL=Etiqueta y Porcentaje Total'+#13+
...
'OUTLINE=Borde'+#13+
'PRINT PREVIEW=Impresión Prelim.'+#13+
'ANIMATED=Animado'+#13+
'ALLOW=Permitir'+#13+
'DASH=Líneas'+#13+
'DOT=Puntos'+#13+
...
'COMPRESSION LEVEL=Nivel de Compresión'+#13+
'COMPRESSION=Compresión'+#13+
'PATTERNS=Tramas'+#13+
'LABEL=Etiqueta'+#13+
'GROUP SLICES=Agrupar porciones'+#13+
'EXPLODE BIGGEST=Separar mayor porción'+#13+
...
'ANGLE INCREMENT=Angulo de Incremento'+#13+
'RADIUS INCREMENT=Incremento de Radio'+#13+
...
'RESET MARGINS=Márgenes Defecto'+#13+
'PRINT=Imprimir'+#13+
...
'TAB=Tabulación'+#13+
'SPACE=Espacio'+#13+
'ROUND RECTANGLE=Rectángulo Redondeado'+#13+
'BOTTOM=Abajo'+#13+
'RIGHT=Derecha'+#13+
'C PEN=Lápiz C'+#13+
'R PEN=Lápiz R'+#13+
...
'DESIRED INCREMENT=Incremento deseado'+#13+
'(INCREMENT)=(Incremento)'+#13+
...
'MARK=Marcas'+#13+
'ROUND FIRST=Redondar primera'+#13+
...
'FONT 3D DEPTH=Profundidad Fuente &3D'+#13+
'NORMAL=Normal'+#13+
...
'SELECT A TEXT LABELS FIELD=Seleccione el campo de etiquetas'+#13+
...
'NEW SERIES TITLE:=Nuevo título:'+#13+
...
'TEECHART LANGUAGE SELECTOR=Lenguajes de TeeChart'#13+
...
;
  ...
  TeeLanguage:=TeeBulgarianLanguage;
  TeeBulgarianConstants;
end;


initialization
finalization
  FreeAndNil(TeeBulgarianLanguage);
end.
