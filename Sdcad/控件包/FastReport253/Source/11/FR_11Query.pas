...
uses
  ...
  Menus, Dialogs, FR_Class, FR_Pars, DB, 11Query, FR_11Table, FR_DBUtils;

type
  Tfr11Query = class(Tfr11DataSet)
  private
    FQuery: T11Query;
  ...
  public
    ...
    property Query: T11Query read FQuery;
...
uses
  FR_Utils, FR_Const, FR_DBSQLEdit, FR_11QueryParam
{$IFDEF QBUILDER}  
, FR_11QB, QBuilder
{$ENDIF} 
{$IFDEF Delphi6}
...
constructor Tfr11Query.Create;
begin
  inherited Create;
  FQuery := T11Query.Create(frDialogForm);
  ...
  Component := FQuery;
  BaseName := 'Query';
  Bmp.LoadFromResourceName(hInstance, 'FR_11QUERY');
end;

destructor Tfr11Query.Destroy;
begin
  FParams.Free;
  inherited Destroy;
end;

procedure Tfr11Query.DefineProperties;
...
procedure Tfr11Query.SetPropValue(Index: String; Value: Variant);
var
  d: TDataset;
begin
...
function Tfr11Query.GetPropValue(Index: String): Variant;
begin
  Index := AnsiUpperCase(Index);
...
function Tfr11Query.DoMethod(MethodName: String; Par1, Par2, Par3: Variant): Variant;
begin
...
procedure Tfr11Query.LoadFromStream(Stream: TStream);
var
  s: String;
begin
  FFixupList.Clear;
...
procedure Tfr11Query.SaveToStream(Stream: TStream);
begin
  LVersion := 1;
...
procedure Tfr11Query.Loaded;
begin
  Prop['DataSource'] := FFixupList['DataSource'];
  inherited Loaded;
end;

procedure Tfr11Query.SQLEditor(Sender: TObject);
begin
...
{$IFDEF QBUILDER} 
    QBEngine := TfrQB11Engine.Create(nil);
    TfrQB11Engine(QBEngine).Query := FQuery;
...
procedure Tfr11Query.ParamsEditor(Sender: TObject);
var
  Params: TParams;
...
begin
  ...
  begin
    ...
    with Tfr11ParamsForm.Create(nil) do
    begin
      QueryComp := Self;
      Query := FQuery;
...
function Tfr11Query.GetParamKind(Index: Integer): TfrParamKind;
begin
  Result := pkValue;
...
procedure Tfr11Query.SetParamKind(Index: Integer; Value: TfrParamKind);
begin
...
function Tfr11Query.GetParamText(Index: Integer): String;
begin
  Result := '';
...
procedure Tfr11Query.SetParamText(Index: Integer; Value: String);
begin
  if ParamKind[Index] = pkValue then
    FParams[FQuery.Params[Index].Name] := Value;
end;

procedure Tfr11Query.ReadParams(Stream: TStream);
var
  i: Integer;
  w, n: Word;
begin
...
procedure Tfr11Query.WriteParams(Stream: TStream);
var
  i: Integer;
  w: Word;
begin
...
procedure Tfr11Query.BeforeOpenQuery(DataSet: TDataSet);
var
  i: Integer;
  SaveView: TfrView;
  SavePage: TfrPage;
...
initialization
  Bmp := TBitmap.Create;
  Bmp.LoadFromResourceName(hInstance, 'FR_11QUERYCONTROL');
  frRegisterControl(Tfr11Query, Bmp, IntToStr(SInsertQuery));

finalization
  frUnRegisterObject(Tfr11Query);
  Bmp.Free;
