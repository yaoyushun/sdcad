...
uses
  ...
  StdCtrls, FR_Ctrls, ExtCtrls, DB, 11Query, FR_11Query;

type
  Tfr11ParamsForm = class(TForm)
    GroupBox1: TGroupBox;
    Label2: TLabel;
    ParamsLB: TListBox;
    TypeCB: TComboBox;
...
function Tfr11ParamsForm.CurParam: Integer;
var
  i: Integer;
  s: String;
begin
...
procedure Tfr11ParamsForm.FormShow(Sender: TObject);
var
  i: Integer;
begin
...
procedure Tfr11ParamsForm.FormHide(Sender: TObject);
begin
  if ValueE.Enabled then ValueEExit(nil);
end;

procedure Tfr11ParamsForm.ParamsLBClick(Sender: TObject);
var
  i: Integer;
begin
  TypeCB.ItemIndex := -1;
...
procedure Tfr11ParamsForm.ValueEExit(Sender: TObject);
begin
  QueryComp.ParamText[CurParam] := ValueE.Text;
end;

procedure Tfr11ParamsForm.TypeCBChange(Sender: TObject);
begin
...
procedure Tfr11ParamsForm.ValueRBClick(Sender: TObject);
begin
  if FBusy then Exit;
  ValueE.Enabled := True;
...
procedure Tfr11ParamsForm.AssignRBClick(Sender: TObject);
begin
  if FBusy then Exit;
  ValueE.Text := '';
  ValueE.Enabled := False;
...
procedure Tfr11ParamsForm.Localize;
begin
...
procedure Tfr11ParamsForm.FormCreate(Sender: TObject);
begin
  Localize;
end;

procedure Tfr11ParamsForm.VarSBClick(Sender: TObject);
begin
