...
uses
  ...
  Controls, Forms, Menus, Dialogs, DB, 11Table, FR_DBSet;

type
  Tfr11Dataset = class(TfrNonVisualControl)
  protected
    FDataSet: TDBDataSet;
  ...
  public
    ...
    procedure DefineProperties; override;
    procedure Loaded; override;
    property Table: T11Table read FTable;
  end;


implementation

uses FR_DBUtils, FR_Utils, FR_Const, FR_LEdit, FR_DBFldEditor, FR_11Md
{$IFDEF Delphi6}
, Variants
{$ENDIF};
...
constructor Tfr11DataSet.Create;
begin
  inherited Create;
...
destructor Tfr11DataSet.Destroy;
begin
  FDBDataset.Free;
  FDataSource.Free;
  FDataSet.Free;
...
procedure Tfr11DataSet.DefineProperties;
...
procedure Tfr11DataSet.SetPropValue(Index: String; Value: Variant);
begin
...
function Tfr11DataSet.GetPropValue(Index: String): Variant;
begin
  Index := AnsiUpperCase(Index);
...
function Tfr11DataSet.DoMethod(MethodName: String; Par1, Par2, Par3: Variant): Variant;
begin
...
procedure Tfr11DataSet.ReadFields(Stream: TStream);
var
  i: Integer;
  n: Word;
  s: String;
...
procedure Tfr11DataSet.WriteFields(Stream: TStream);
var
  i: Integer;
  s: String;
  SaveActive: Boolean;
...
procedure Tfr11DataSet.ShowEditor;
begin
  FieldsEditor(nil);
end;

procedure Tfr11DataSet.FieldsEditor(Sender: TObject);
var
  SaveActive: Boolean;
begin
  SaveActive := FDataSet.Active;
...
constructor Tfr11Table.Create;
begin
  inherited Create;
  FTable := T11Table.Create(frDialogForm);
  FDataSet := FTable;
  FDataSource.DataSet := FDataSet;

  Component := FTable;
  BaseName := 'Table';
  Bmp.LoadFromResourceName(hInstance, 'FR_11TABLE');
end;

procedure Tfr11Table.DefineProperties;
...
procedure Tfr11Table.SetPropValue(Index: String; Value: Variant);
var
  d: TDataset;
begin
  inherited SetPropValue(Index, Value);
...
function Tfr11Table.GetPropValue(Index: String): Variant;
begin
  Index := AnsiUpperCase(Index);
  Result := inherited GetPropValue(Index);
  if Result <> Null then Exit;
...
procedure Tfr11Table.LoadFromStream(Stream: TStream);
begin
  FFixupList.Clear;
...
procedure Tfr11Table.SaveToStream(Stream: TStream);
begin
  LVersion := 1;
...
procedure Tfr11Table.Loaded;
begin
  Prop['MasterSource'] := FFixupList['MasterSource'];
  inherited Loaded;
end;

procedure Tfr11Table.JoinEditor(Sender: TObject);
begin
  with Tfr11FieldsLinkForm.Create(nil) do
  begin
...
initialization
  Bmp := TBitmap.Create;
  Bmp.LoadFromResourceName(hInstance, 'FR_11TABLECONTROL');
  frRegisterControl(Tfr11Table, Bmp, IntToStr(SInsertTable));

finalization
  frUnRegisterObject(Tfr11Table);
  Bmp.Free;
