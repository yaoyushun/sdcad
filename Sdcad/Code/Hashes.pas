...
}
{
 ...
 STL中也有map，hashmap，当然map一般是使用平衡BST来实现的，但是外部使用接口和hash table差不多。
 dict和list，scalar（string和numbers）组成了python，perl等脚本语言中基本数据元素。
 在各种程序应用中，相当大部分计算是在处理字符串，很多情况下，hash table会给字符串处理提供极大的方便。
 ...
 我要推荐的是Hashes.pas，作者是Ciaran McCreesh，早就停止更新了，它提供了3个类，
 TIntegerHash，TObjectHash，TStringHash，从名字就可以知道它们k-v对中的value类型分别为integer，object,string。
 我在n个项目中都使用了它，用法很简单
 uses Hashes
  ...
  type
    ...
    TObjectHash = class(THash)
      ...
      private
        FAutoFree : boolean;
      public
        constructor Create(bAutoFree:boolean); reintroduce; virtual;
...
begin
  for i := 0 to High(self.f_Items) do
    if FAutoFree and (Assigned(self.f_Items[i])) then
      self.f_Items[i].Free;
  SetLength(self.f_Items, 0);
end;

constructor TObjectHash.Create(bAutoFree:boolean);
begin
  inherited Create;
  FAutoFree := bAutoFree;
end;
