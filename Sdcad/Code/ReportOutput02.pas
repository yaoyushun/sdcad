...
function TfrmCharge02.getOneDrillStratumDepth(prj_no,drill_no:string;lstStra_NO,lstSub_NO:TStringList):Integer;
var
  i:integer;
  intRow:integer;
  intCol:integer;
...
//            'where drl_no in (select drl_no from drills where '+m_JueSuan_FieldName+'=0 and prj_no='''+prj_no+''') '+
//            'and prj_no='''+prj_no+''' and drl_no='''+stringReplace(drill_no ,'''','''''',[rfReplaceAll])+'''';
//    ADOMax.Open;
...
begin
  ...
  ASubNoList.Clear;
  ACategoryList.Clear;
  strSQL:='SELECT s.id,s.prj_no,s.stra_no,s.sub_no,s.stra_category,sc.category FROM stratum_description s LEFT JOIN stratumcategory02 sc on s.stra_category=sc.id '
...
const
  ...
  TuLeiBie:TTuLeiBie = ('0~10m' ,'10~20m','20~30m','30~40m','40~50m','50~60m','60~80m','80~100m','100~120m','120~140m','140~160m');
...
begin
    ...
    begin
      ...
      //工程中每一级深度范围，按所有层数循环加
      for itmp := 10 downto 0 do  //itmp=0 0~10m ,1 10~20m, 2 20~30m,  3 30~40m,  4 40~50m,  5 50~60m,  6 60~80m,  7 80~100m,  8 100~120m,  9 120~140m,  10 140~160m,
      begin
      //合并单元格,深度范围
    ...
    begin
        ...
        //按土层号来统计深度
        for itmp := 0 to 11*lstStratumNo.Count-1 do  //itmp=0 0~10m ,1 10~20m, 2 20~30m,  3 30~40m,  4 40~50m,  5 50~60m,  6 60~80m,  7 80~100m,  8 100~120m,  9 120~140m,  10 140~160m,
        begin
...
//        for itmp := 0 to 10 do  //itmp=0 0~10m ,1 10~20m, 2 20~30m,  3 30~40m,  4 40~50m,  5 50~60m,  6 60~80m,  7 80~100m,  8 100~120m,  9 120~140m,  10 140~160m,
//        begin
//            ExcelApplication1.Cells(iTuLeiBieBeginRow+m_intRecordCount, iTuLeiBieBeginCol+iTuLeiBieColCount*iTmp+1):= FormatFloat('0.00',zhuantanland_one[10*iTmp+0]);
//            ExcelApplication1.Cells(iTuLeiBieBeginRow+m_intRecordCount, iTuLeiBieBeginCol+iTuLeiBieColCount*iTmp+2):= FormatFloat('0.00',zhuantanland_one[10*iTmp+1]);
//            ExcelApplication1.Cells(iTuLeiBieBeginRow+m_intRecordCount, iTuLeiBieBeginCol+iTuLeiBieColCount*iTmp+3):= FormatFloat('0.00',zhuantanland_one[10*iTmp+2]);
//            ExcelApplication1.Cells(iTuLeiBieBeginRow+m_intRecordCount, iTuLeiBieBeginCol+iTuLeiBieColCount*iTmp+4):= FormatFloat('0.00',zhuantanland_one[10*iTmp+3]);
//            ExcelApplication1.Cells(iTuLeiBieBeginRow+m_intRecordCount, iTuLeiBieBeginCol+iTuLeiBieColCount*iTmp+5):= FormatFloat('0.00',zhuantanland_one[10*iTmp+4]);
//            ExcelApplication1.Cells(iTuLeiBieBeginRow+m_intRecordCount, iTuLeiBieBeginCol+iTuLeiBieColCount*iTmp+6):= FormatFloat('0.00',zhuantanland_one[10*iTmp+5]);
//            ExcelApplication1.Cells(iTuLeiBieBeginRow+m_intRecordCount, iTuLeiBieBeginCol+iTuLeiBieColCount*iTmp+7):= FormatFloat('0.00',zhuantanland_one[10*iTmp+6]);
//            ExcelApplication1.Cells(iTuLeiBieBeginRow+m_intRecordCount, iTuLeiBieBeginCol+iTuLeiBieColCount*iTmp+8):= FormatFloat('0.00',zhuantanland_one[10*iTmp+7]);
//        end;
    ...
    begin
      ...
      ExcelApplication1.Cells(34,4+intTag):=zhuantanland_all[50];     //钻探,陆上工作量(50~60米,I)
      ExcelApplication1.Cells(35,4+intTag):=zhuantanland_all[51];     //钻探,陆上工作量(50~60米,II)
      ExcelApplication1.Cells(36,4+intTag):=zhuantanland_all[52];     //钻探,陆上工作量(50~60米,III)
      ExcelApplication1.Cells(37,4+intTag):=zhuantanland_all[53];     //钻探,陆上工作量(50~60米,IV)
      ExcelApplication1.Cells(38,4+intTag):=zhuantanland_all[54];     //钻探,陆上工作量(50~60米,V)

      ExcelApplication1.Cells(39,4+intTag):=zhuantanland_all[60];     //钻探,陆上工作量(60~80米,I)
      ExcelApplication1.Cells(40,4+intTag):=zhuantanland_all[61];     //钻探,陆上工作量(60~80米,II)
      ExcelApplication1.Cells(41,4+intTag):=zhuantanland_all[62];     //钻探,陆上工作量(60~80米,III)
      ExcelApplication1.Cells(42,4+intTag):=zhuantanland_all[63];     //钻探,陆上工作量(60~80米,IV)
      ExcelApplication1.Cells(43,4+intTag):=zhuantanland_all[64];     //钻探,陆上工作量(60~80米,V)

      ExcelApplication1.Cells(44,4+intTag):=zhuantanland_all[70];     //钻探,陆上工作量(80~100米,I)
      ExcelApplication1.Cells(45,4+intTag):=zhuantanland_all[71];     //钻探,陆上工作量(80~100米,II)
      ExcelApplication1.Cells(46,4+intTag):=zhuantanland_all[72];     //钻探,陆上工作量(80~100米,III)
      ExcelApplication1.Cells(47,4+intTag):=zhuantanland_all[73];     //钻探,陆上工作量(80~100米,IV)
      ExcelApplication1.Cells(48,4+intTag):=zhuantanland_all[74];     //钻探,陆上工作量(80~100米,V)

      ExcelApplication1.Cells(49,4+intTag):=zhuantanland_all[80];     //钻探,陆上工作量(100~120米,I)
      ExcelApplication1.Cells(50,4+intTag):=zhuantanland_all[81];     //钻探,陆上工作量(100~120米,II)
      ExcelApplication1.Cells(51,4+intTag):=zhuantanland_all[82];     //钻探,陆上工作量(100~120米,III)
      ExcelApplication1.Cells(52,4+intTag):=zhuantanland_all[83];     //钻探,陆上工作量(100~120米,IV)
      ExcelApplication1.Cells(53,4+intTag):=zhuantanland_all[84];     //钻探,陆上工作量(100~120米,V)

      ExcelApplication1.Cells(54,4+intTag):=zhuantanland_all[90];     //钻探,陆上工作量(120~140米,I)
      ExcelApplication1.Cells(55,4+intTag):=zhuantanland_all[91];     //钻探,陆上工作量(120~140米,II)
      ExcelApplication1.Cells(56,4+intTag):=zhuantanland_all[92];     //钻探,陆上工作量(120~140米,III)
      ExcelApplication1.Cells(57,4+intTag):=zhuantanland_all[93];     //钻探,陆上工作量(120~140米,IV)
      ExcelApplication1.Cells(58,4+intTag):=zhuantanland_all[94];     //钻探,陆上工作量(120~140米,V)

      ExcelApplication1.Cells(59,4+intTag):=zhuantanland_all[100];     //钻探,陆上工作量(140~160米,I)
      ExcelApplication1.Cells(60,4+intTag):=zhuantanland_all[101];     //钻探,陆上工作量(140~160米,II)
      ExcelApplication1.Cells(61,4+intTag):=zhuantanland_all[102];     //钻探,陆上工作量(140~160米,III)
      ExcelApplication1.Cells(62,4+intTag):=zhuantanland_all[103];     //钻探,陆上工作量(140~160米,IV)
      ExcelApplication1.Cells(63,4+intTag):=zhuantanland_all[104];     //钻探,陆上工作量(140~160米,V)
    ...
    begin
      ...
      ExcelApplication1.Cells(40,11):=zhuantanland_all[50];     //钻探,(50~60米,I)
      ExcelApplication1.Cells(41,11):=zhuantanland_all[51];     //钻探,(50~60米,II)
      ExcelApplication1.Cells(42,11):=zhuantanland_all[52];     //钻探,(50~60米,III)

      ExcelApplication1.Cells(43,11):=zhuantanland_all[60];     //钻探,(60~80米,I)
      ExcelApplication1.Cells(44,11):=zhuantanland_all[61];     //钻探,(60~80米,II)
      ExcelApplication1.Cells(45,11):=zhuantanland_all[62];     //钻探,(60~80米,III)

      ExcelApplication1.Cells(46,11):=zhuantanland_all[70];     //钻探,(80~100米,I)
      ExcelApplication1.Cells(47,11):=zhuantanland_all[71];     //钻探,(80~100米,II)
      ExcelApplication1.Cells(48,11):=zhuantanland_all[72];     //钻探,(80~100米,III)

      ExcelApplication1.Cells(49,11):=zhuantanland_all[80];     //钻探,(100~120米,I)
      ExcelApplication1.Cells(50,11):=zhuantanland_all[81];     //钻探,(100~120米,II)
      ExcelApplication1.Cells(51,11):=zhuantanland_all[82];     //钻探,(100~120米,III)
        ...
        begin
            ...
            begin
                ...
                begin 
                    ...
                    case ADO02.fieldByName('stra_category').AsInteger of
                    0:
...
begin
    ...
    ExcelApplication1.Cells(50,17):=dblCount[2];
    ExcelApplication1.Cells(52,17):=dblCount[3];
...
begin
    ...
    ExcelApplication1.Cells(56,11):=intguanbiao[1];
    ExcelApplication1.Cells(57,11):=intguanbiao[2];
    ExcelApplication1.Cells(58,11):=intguanbiao[3];
    ExcelApplication1.Cells(59,11):=intguanbiao[4];
    ExcelApplication1.Cells(60,11):=intguanbiao[5];
    ExcelApplication1.Cells(61,11):=intguanbiao[6];
    ExcelApplication1.Cells(62,11):=intguanbiao[7];
    ExcelApplication1.Cells(63,11):=intguanbiao[8];
    ExcelApplication1.Cells(64,11):=intguanbiao[9];
...
begin
    ADO02.Close;
    ADO02.SQL.Text:='select * from stratum where drl_no in (select drl_no from drills where '+m_JueSuan_FieldName+'=0 and prj_no='''+ strProjectCode + ''' and d_t_no=3) and prj_no='''+strProjectCode+''' order by prj_no,drl_no,stra_depth';
    ADO02.Open;
    dblCount:=0;
    strdrl_no:='';
    while not ADO02.Recordset.eof do
    ...
    xlsFileName:=ERP1.Text;
    ExcelWorkbook1:=ExcelApplication1.workBooks.Open(xlsFileName);
    ExcelApplication1.Cells(59,17):=(dblWhorlValue[1]+dblWhorlValue[2]+dblWhorlValue[3]);
    ExcelWorkbook1.Save;
    ExcelWorkbook1.Close;
    ExcelApplication1.Quit;
    ExcelApplication1:=Unassigned;
...
begin
    ...
    ExcelApplication1.Cells(63,17):=intCount;
    ExcelWorkbook1.Save;
    ExcelWorkbook1.Close;
    ExcelApplication1.Quit;
    ExcelApplication1:=Unassigned;
...
begin
    ...
    //begin
        copyfile(pChar(ExtractFileDir(Application.Exename)+'\XLS\02_new.xls'),PChar(ERP1.text),false);
        //yys 2011/06/29  土的类别统计_093.xls 辅助决算用，为了看起来直观
        m_FullFileName_TuLeiBieTongJi := strPath +'\'+g_ProjectInfo.Prj_no+'土的类别统计_02.xlsx';
        CopyFile(pChar(ExtractFileDir(Application.Exename)+'\XLS\土的类别统计_02.xlsx'),PChar(m_FullFileName_TuLeiBieTongJi),false);
    //end;
...
procedure TfrmCharge02.setJueSuanFieldName(iIndex: integer);
begin
  if iIndex<=0 then
...
function TfrmCharge02.getZhongLiChuTanInfo(
