...
uses
  ...
  Dialogs, StdCtrls, Buttons, ADODB, DB, FR_DSet, FR_DBSet, FR_Class,
  ExtCtrls;
...
type
  ...
  private
    ...
    //重力触探数据分层
    procedure ZhongLiChuTanShuJuFenCeng;
    //重力触探数据计算并剔除
    procedure ZhongLiChuTanShuJuJiSuan;
  public
    { Public declarations }
  end;
...
procedure GetDrillNo(var AStingList: TStringList; strTableName:string);
begin
  AStingList.Clear;
...
//2005/04/22 yys edit
      sql.Add('SELECT DISTINCT drl_no FROM ' + strTableName );
      //2008/11/21 yys edit 增加钻孔检查条件，是否参与统计
...
begin
  ...
  {******最小值、样本值等插入特征数表TeZhengShuTmp ****}
    ...
    //开始插入标准值
    strSQL:= '';
    strSQL:=strSetValue + 'spt_real_num='+FloatToStr(real_num.BiaoZhunZhi) +','
      + 'spt_amend_num='+FloatToStr(amend_num.BiaoZhunZhi) + strWhereClause + '7';
    if not Update_oneRecord(MainDataModule.qrySectionTotal, strSQL) then
      continue;
  end;
  FreeStringList;
...
procedure TWuLiLiXueForm.frReport1GetValue(const ParName: String;
  var ParValue: Variant);
begin
...
begin
  ...
  {******把平均值、标准差、变异系数、最大值、**********}
  {******最小值、样本值、标准值插入特征数表TeZhengShuTmp ****}
    //
    ...
    //开始插入标准值
    strSQL:= '';
    strSQL:=strSetValue + 'cpt_qc='+FloatToStr(qc.BiaoZhunZhi) +','
      + 'cpt_fs='+FloatToStr(fs.BiaoZhunZhi) +','
      + 'cpt_ps='+FloatToStr(ps.BiaoZhunZhi) + strWhereClause + '7';
    if not Update_oneRecord(MainDataModule.qrySectionTotal, strSQL) then
      continue;
  end;
  FreeStringList;
...
begin
  try
    ...
    BiaoGuanShuJuJiSuan;
    //重力触探数据分层
    ZhongLiChuTanShuJuFenCeng;
    //重力触探数据计算并剔除
    ZhongLiChuTanShuJuJiSuan;
  finally
...
procedure TWuLiLiXueForm.ZhongLiChuTanShuJuFenCeng;
var
  iDrillCount,iStratumCount:integer;
  strSQL: string;
  lstDrillsNo,lstStratumNo,lstSubNo,
...
begin
  ...
  try
    //清除工程中所有重力触探表的记录中层和亚层的信息
    strSQL:= 'UPDATE dpt set stra_no=null,sub_no='+''''+'0'+''''+' WHERE prj_no='
      +''''+g_ProjectInfo.prj_no_ForSQL+'''';
      with MainDataModule.qrySPT do
      begin
        close;
...
procedure TWuLiLiXueForm.ZhongLiChuTanShuJuJiSuan;
var
    //层号          亚层号
  lstStratumNo, lstSubNo, lstEa_name: TStringList;
  real_num : TAnalyzeResult;  //实测击数
...
begin
  ...
  begin
    strSub_no := stringReplace(lstSubNo.Strings[i] ,'''','''''',[rfReplaceAll]);
    strStratum_No := lstStratumNo.Strings[i];
    strSQL:='SELECT prj_no, drl_no, real_num1+real_num2+real_num3 real_num, amend_num FROM dpt '
      +' WHERE prj_no='
      +''''+strPrj_No+''''
      +' AND stra_no='+''''+strStratum_No+''''
      +' AND sub_no='+''''+strSub_no+'''';
  ...
  {******最小值、样本值等插入特征数表TeZhengShuTmp ****}
    ...
    //开始插入平均值
    strSQL:= '';
    strSQL:=strSetValue + 'dpt_real_num='+FloatToStr(real_num.PingJunZhi) +','
      + 'dpt_amend_num='+FloatToStr(amend_num.PingJunZhi) + strWhereClause + '1';
    if not Update_oneRecord(MainDataModule.qrySectionTotal, strSQL) then 
      continue;
      
    //开始插入标准差
    strSQL:= '';
    strSQL:=strSetValue + 'dpt_real_num='+FloatToStr(real_num.BiaoZhunCha) +','
      + 'dpt_amend_num='+FloatToStr(amend_num.BiaoZhunCha) + strWhereClause + '2';
    if not Update_oneRecord(MainDataModule.qrySectionTotal, strSQL) then 
      continue;
      
    //开始插入变异系数
    strSQL:= '';
    strSQL:=strSetValue + 'dpt_real_num='+FloatToStr(real_num.BianYiXiShu) +','
      + 'dpt_amend_num='+FloatToStr(amend_num.BianYiXiShu) + strWhereClause + '3';
    if not Update_oneRecord(MainDataModule.qrySectionTotal, strSQL) then 
      continue;
      
    //开始插入最大值
    strSQL:= '';
    strSQL:=strSetValue + 'dpt_real_num='+FloatToStr(real_num.MaxValue) +','
      + 'dpt_amend_num='+FloatToStr(amend_num.MaxValue) + strWhereClause + '4';
    if not Update_oneRecord(MainDataModule.qrySectionTotal, strSQL) then 
      continue;
      
    //开始插入最小值
    strSQL:= '';
    strSQL:=strSetValue + 'dpt_real_num='+FloatToStr(real_num.MinValue) +','
      + 'dpt_amend_num='+FloatToStr(amend_num.MinValue) + strWhereClause + '5';
    if not Update_oneRecord(MainDataModule.qrySectionTotal, strSQL) then 
      continue;
      
    //开始插入样本值
    strSQL:= '';
    strSQL:=strSetValue + 'dpt_real_num='+FloatToStr(real_num.SampleNum) +','
      + 'dpt_amend_num='+FloatToStr(amend_num.SampleNum) + strWhereClause + '6';
    if not Update_oneRecord(MainDataModule.qrySectionTotal, strSQL) then
      continue;

    //开始插入标准值
    strSQL:= '';
    strSQL:=strSetValue + 'dpt_real_num='+FloatToStr(real_num.BiaoZhunZhi) +','
      + 'dpt_amend_num='+FloatToStr(amend_num.BiaoZhunZhi) + strWhereClause + '7';
    if not Update_oneRecord(MainDataModule.qrySectionTotal, strSQL) then
      continue;
  end;
  FreeStringList;
