...
//取得样品数所对应的标准值
//公式:f= 1- (1.704/sqrt(样本数) + 4.678/sqr(样本数)) *变异系数)))
...
//yys 2005/06/15
  ...
  begin
    //yys edit 2012/02/24
    //勘察院要求变异系统除了原本是3位小数的，其他都按2位小数来做
  ...
  begin
    //2011/03/09 勘察院修改要求，所有字段都要加上标准差和变异系数
    //aAnalyzeResult.BiaoZhunCha := -1;
    //aAnalyzeResult.BianYiXiShu := -1;
  end;
//yys edit 2009/12/29工勘院修改要求，只有土样的凝聚力和摩擦角(这两种在另外的函数GetTeZhengShuGuanLian中计算)还有非土样的标贯、静探需要计算标准值，其他都不在计算标准值，同时报表上这些不要计算标准值的要空白显示。
