...
BEGIN_MESSAGE_MAP(CMiniCADView, CView)
	...
	ON_WM_KEYDOWN()
	ON_COMMAND(ID_PROPERTYEDIT, OnPropertyedit)
	ON_COMMAND(ID_COPY, OnCopy)
	ON_COMMAND(ID_PASTE, OnPaste)
	ON_UPDATE_COMMAND_UI(ID_COPY, OnUpdateCopy)
	ON_UPDATE_COMMAND_UI(ID_PROPERTYEDIT, OnUpdatePropertyedit)
...
void CMiniCADView::OnPropertyedit() 
{
	// TODO: Add your command handler code here
	CMiniCADDoc* pDoc = GetDocument();
	CDlgTextEdit textDlg;
	textDlg.m_pMText = (CADMText *)pDoc->m_Graphics.m_pRSelEntity;
	textDlg.m_pView = this;
	textDlg.DoModal();
}

void CMiniCADView::OnCopy() 
{
	// TODO: Add your command handler code here
	CMiniCADDoc* pDoc = GetDocument();
	if (pDoc->m_Graphics.m_pRSelEntity != NULL)
		pDoc->m_Graphics.m_pCopyEntity = pDoc->m_Graphics.m_pRSelEntity;
}

void CMiniCADView::OnPaste() 
{
	// TODO: Add your command handler code here
	CMiniCADDoc* pDoc = GetDocument();
	if (pDoc->m_Graphics.m_pRSelEntity != NULL)
	{
		CADMText *pOrgMText = (CADMText *)pDoc->m_Graphics.m_pRSelEntity;
		CADMText *pMText = new CADMText();
		pDoc->m_Graphics.m_Entities.Add((CObject*)pMText);
		pMText->m_nLayer = pOrgMText->m_nLayer;
		pMText->m_Location.x = pOrgMText->m_Location.x+3.0;
		pMText->m_Location.y = pOrgMText->m_Location.y-1.0;
		pMText->m_Text = pOrgMText->m_Text;
		strcpy(pMText->m_Font,pOrgMText->m_Font);
		pMText->m_Align = pOrgMText->m_Align;
		pMText->m_Height = pOrgMText->m_Height;
		ReDraw();
	}
}

void CMiniCADView::OnUpdateCopy(CCmdUI* pCmdUI) 
{
	// TODO: Add your command update UI handler code here
	CMiniCADDoc* pDoc = GetDocument();
	pCmdUI->Enable(pDoc->m_Graphics.m_bRSelected);
}

void CMiniCADView::OnUpdatePropertyedit(CCmdUI* pCmdUI) 
{
	// TODO: Add your command update UI handler code here
	CMiniCADDoc* pDoc = GetDocument();
	pCmdUI->Enable(pDoc->m_Graphics.m_bRSelected);	
...
void CMiniCADView::OnUpdatePaste(CCmdUI* pCmdUI) 
{
	// TODO: Add your command update UI handler code here
	CMiniCADDoc* pDoc = GetDocument();
	pCmdUI->Enable((pDoc->m_Graphics.m_pCopyEntity != NULL));
}
