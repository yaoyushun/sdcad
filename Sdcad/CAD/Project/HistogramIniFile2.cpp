...
void CHistogramIniFile2::DrawRect(double left,double top,double width,double height,int curLayerIndex)
{
	CADPolyline* pPolyline;
	ADPOINT* pPoint;
	pPolyline=new CADPolyline();
...
//	CADMText* pMText;
	...
	m_pGraphics->m_Entities.Add((CObject*)pMText);
	pMText->m_nLayer=layerIndex;
	pMText->m_Text="标准贯入试验直方图";//"标准贯入试验曲线";
	pMText->m_Height=m_FontHeight;
	pMText->m_Align=AD_MTEXT_ATTACH_TOPCENTER;
...
//原位测试击数-----------------------------------------------
	...
	while(pszTemp)
	{
		...
		DrawRect(GetGridColLeft(curCol+1),sampleTop + 1.0,rulerWidth,2.0,curLayerIndex);
