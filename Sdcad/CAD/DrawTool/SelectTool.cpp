...
void CSelectTool::OnRButtonUp(_CVIEW* pView, UINT nFlags, CPoint& point)
{
	...
	pDoc->m_Graphics.m_bRSelected = false;
	CADEntity *pEntity = NULL;
	//updated on 200811/1
	//pEntity = pDoc->m_Graphics.SelectEntity(point);
	pEntity = pDoc->m_Graphics.RightSelectEntity(point);
	//if (pEntity==NULL)return;
	if (pEntity!=NULL && pEntity->GetType()==AD_ENT_MTEXT)
	{
		pDoc->m_Graphics.m_pRSelEntity = pEntity;
		pDoc->m_Graphics.m_bRSelected = true;
		/*CDlgTextEdit textDlg;
		textDlg.m_pMText = (CADMText *)pEntity;
		textDlg.m_pView = pView;
		textDlg.DoModal();*/
	}
	else if(pEntity!=NULL && pEntity->GetType()==AD_ENT_SPLINE)
	{
		CMenu menu;	
