#include "stdafx.h"
#include "HistogramIniFile3.h"
#define LegendWidth 0.8//unit cm
#include "..\FunctionLib\Function_System.h"

CHistogramIniFile3::CHistogramIniFile3(CADLayerGroup* pLayerGroup,CADGraphics* pGraphics)
{
...
//initialize===============================================
	...
	m_FontHeight=3.5;
	m_NumFontH=2.5;
	m_Layer0Top = 188.5 + 30 - 5.7;//188.5;
	//m_Row1Height=25;
	m_Row1Height = 32;
	m_LeftMargin=10;
	m_TopMargin=64;
...
//==========================================================
	...
	m_FootGridColWidth[curFootColIndex++]=180;
...
CHistogramIniFile3::~CHistogramIniFile3()
{
	delete[] m_GridColWidth;
	delete[] m_FootGridColWidth;
}

void CHistogramIniFile3::FileImport(LPCTSTR lpFilename)
{
...
//===========================================================
	...
	CreateChartGrid(GridWidth,280,m_PaperDeep);

	//updated on 20081111
	CreateChartHeader(0,0);
	CreateChartHeader(GridWidth,2);

	CreateChartFooter(0,280,m_PaperDeep);
//	CreateChartFrame();

	CreateGrid(0,0);
	CreateBeatNum(lpFilename, 0,0);
	CreateBeatNum2(lpFilename, 0,0);
	CreateGrid(GridWidth,2);
	CreateBeatNum(lpFilename, GridWidth ,2);
	CreateBeatNum2(lpFilename, GridWidth ,2);
	CreateChartFooter(lpFilename);
}

double CHistogramIniFile3::GetGridColLeft(int i)
{
	double left;
	left=m_GridLeft;
	for(int j=0;j<i-1;j++)
...
CADBlock* CHistogramIniFile3::CreateLayerBlock(CPerHoleLayer* pHoleLayer,double left)
{
...
void CHistogramIniFile3::CreateGrid(double left,int num)
{
	int i;
	CADLine* pLine;
	CADMText* pMText;
...
/*	int nLayer1=m_pLayerGroup->indexOf("1");
	...
	double curLayerDeep;
	double line1Left=left + m_GridLeft;
	double line1Right=left + GetGridColLeft(5)+8;
	double line2Left=line1Right+4;
	double line2Right=left + GetGridColLeft(6);
	double line3Left=left + GetGridColLeft(6);
	double line3Right=left + GetGridColLeft(7);
...
/*	pLine=new CADLine();	
	...
	CADBlock* pBlock=CreateLayerBlock(pHoleLayer,0);
	if(pBlock)
	{
	...
	while(pszTemp)
	{
		...
		pBlock=CreateLayerBlock(pHoleLayer,0);
		if(pBlock)
		{
			CADInsert* pInsert=new CADInsert();
			m_pGraphics->m_Entities.Add((CObject*)pInsert);
...
//===========================================================================================
	curLayerIndex = 1;
	//curLayer=nLayer1;
	GetPrivateProfileString(keyName,"层底深度","",str,maxStrLen,m_FileName);
	pszTemp = strtok(str, ",");
...
//===========================================================================================
	curLayerIndex = 1;
	GetPrivateProfileString(keyName,"层底标高","",str,maxStrLen,m_FileName);
	pszTemp = strtok(str, ",");
...
//1.岩土描述
	...
	for(i=0;i<m_Holelayers.GetSize();i++)
	{
		...
		pHoleLayer=(CPerHoleLayer*)m_Holelayers.GetAt(i);
		GetPrivateProfileString(keyName3,tempStr,"",str,maxStrLen,m_FileName);
...
//===========================================================================================
	...
	int m_nsampling;
	m_nsampling=::GetPrivateProfileInt(keyName2,"个数",0,m_FileName);
	...
	{
		tempStr.Format("%d",(i+1));
		GetPrivateProfileString(keyName2,tempStr,"",str,maxStrLen,m_FileName);
...
//===========================================================================================
	...
	//GetPrivateProfileString(keyName4,"锥尖阻力平均值","",str,maxStrLen,m_FileName);
	GetPrivateProfileString(keyName4,"fao","",str,maxStrLen,m_FileName);
	pszTemp = strtok(str, ",");
...
//===========================================================================================
	...
	//GetPrivateProfileString(keyName4,"侧壁摩阻力平均值","",str,maxStrLen,m_FileName);
	GetPrivateProfileString(keyName4,"qik","",str,maxStrLen,m_FileName);
	pszTemp = strtok(str, ",");
...
void CHistogramIniFile3::CreateChartHeader(double left,int num)
{
...
/*	pMText=new CADMText();
	...
	pMText->m_Height=m_FontHeight;
	//pMText->m_Width=3;
	pMText->m_Location.x= left + 29+2;
	pMText->m_Location.y = curY;// - 10;

	float subNum = 35;
//里程桩号,added on 2007/6/22
	CString strMiliage;
	GetPrivateProfileString(keyName,"里程桩号","",str,255,m_FileName);
	strMiliage = str;
	...
	pMText->m_nLayer=m_pLayerGroup->indexOf("0");
	pMText->m_Align=AD_MTEXT_ATTACH_TOPLEFT;
	GetPrivateProfileString(keyName,"孔口坐标X","",str,255,m_FileName);
	pMText->m_Text="X=";
	pMText->m_Text+=str;
	strcpy(pMText->m_Font,ENFONTNAME);
	pMText->m_Height=m_FontHeight;
	pMText->m_Location.x= left + 130+2-subNum;
	pMText->m_Location.y=curY;
	...
	pMText->m_nLayer=m_pLayerGroup->indexOf("0");
	pMText->m_Align=AD_MTEXT_ATTACH_TOPLEFT;
	GetPrivateProfileString(keyName,"孔口坐标Y","",str,255,m_FileName);
	pMText->m_Text="Y=";
	pMText->m_Text+=str;
	strcpy(pMText->m_Font,ENFONTNAME);
	pMText->m_Height=m_FontHeight;
	pMText->m_Location.x= left + 130+2+25-subNum;
	pMText->m_Location.y=curY;
...
//	pMText->m_Text=pMText->m_Text+"(m)";
	...
	pMText->m_nLayer=m_pLayerGroup->indexOf("0");
	pMText->m_Align=AD_MTEXT_ATTACH_TOPLEFT;
	GetPrivateProfileString(keyName,"初见水位","",str,255,m_FileName);
	pMText->m_Text=str;
	strcpy(pMText->m_Font,ENFONTNAME);
	pMText->m_Height=m_FontHeight;
	pMText->m_Location.x= left + 41;;//left + 227+2;
	pMText->m_Location.y=curY - 20;

	pMText=new CADMText();
	m_pGraphics->m_Entities.Add((CObject*)pMText);
	pMText->m_nLayer=m_pLayerGroup->indexOf("0");
	pMText->m_Align=AD_MTEXT_ATTACH_TOPLEFT;
	GetPrivateProfileString(keyName,"稳定水位","",str,255,m_FileName);
	pMText->m_Text=str;
	strcpy(pMText->m_Font,ENFONTNAME);
	pMText->m_Height=m_FontHeight;
	pMText->m_Location.x= left + 103;;//left + 227+2;
	pMText->m_Location.y= curY - 20;
	...
	pMText->m_nLayer=m_pLayerGroup->indexOf("0");
	pMText->m_Align=AD_MTEXT_ATTACH_TOPLEFT;
	GetPrivateProfileString(keyName,"开工日期","",str,255,m_FileName);
	pMText->m_Text=str;
	strcpy(pMText->m_Font,ENFONTNAME);
	pMText->m_Height=m_FontHeight;
	pMText->m_Location.x= left + 210+2-subNum;
	pMText->m_Location.y=curY;
	...
	pMText->m_nLayer=m_pLayerGroup->indexOf("0");
	pMText->m_Align=AD_MTEXT_ATTACH_TOPLEFT;
	GetPrivateProfileString(keyName,"完工日期","",str,255,m_FileName);
	pMText->m_Text=str;
	strcpy(pMText->m_Font,ENFONTNAME);
	pMText->m_Height=m_FontHeight;
	pMText->m_Location.x= left + 210+2-subNum;
...
//----------------------------------------------------------------------
	...
	pMText->m_Height=m_FontHeight;
	//pMText->m_Width=3;
	pMText->m_Location.x =  left + 51;
	pMText->m_Location.y = 201.5 + 30 - 5;

}

/*void CHistogramIniFile3::CreateChartFrame()
{
	CADPolyline* pPolyline;
	ADPOINT* pPoint;
	pPolyline=new CADPolyline();
...
void CHistogramIniFile3::CreateChartFrame(double left,double top,double holeDeep)
{
	int layerIndex = 0;
	double GridWidth = 0.0;
...
void CHistogramIniFile3::CreateChartGrid(double left,double top,double holeDeep)
{
//	return;
	int layerIndex = 0;
...
//	CADMText* pMText;
	...
	m_pGraphics->m_Entities.Add((CObject*)pMText);
	pMText->m_nLayer=layerIndex;
	pMText->m_Text="Bore         No:";//"钻孔编号：";
	pMText->m_Height=m_FontHeight;
	pMText->m_Align=AD_MTEXT_ATTACH_TOPRIGHT;
	pMText->m_Location.x=left+29;
	pMText->m_Location.y=top-8;//18;
	strcpy(pMText->m_Font,ENFONTNAME);
...
//	CADMText* pMText;
	...
	m_pGraphics->m_Entities.Add((CObject*)pMText);
	pMText->m_nLayer=layerIndex;
	pMText->m_Text="Bore Position:";//"钻孔位置：";
	pMText->m_Height=m_FontHeight;
	pMText->m_Align=AD_MTEXT_ATTACH_TOPRIGHT;
...
//	CADMText* pMText;
	...
	m_pGraphics->m_Entities.Add((CObject*)pMText);
	pMText->m_nLayer=layerIndex;
	pMText->m_Text="Broe Elevation:";//"孔口标高：";
	pMText->m_Height=m_FontHeight;
	pMText->m_Align=AD_MTEXT_ATTACH_TOPRIGHT;
	pMText->m_Location.x=left+130-subNum;
...
//	CADMText* pMText;
	...
	m_pGraphics->m_Entities.Add((CObject*)pMText);
	pMText->m_nLayer=layerIndex;
	pMText->m_Text="Primary Water Level:";//"初见水位（埋深/标高）：";
	pMText->m_Height=m_FontHeight;
	pMText->m_Align=AD_MTEXT_ATTACH_TOPLEFT;
	pMText->m_Location.x=left+11;//left+227;
	pMText->m_Location.y=top-28;//8;
	strcpy(pMText->m_Font,ENFONTNAME);
...
//	CADMText* pMText;
	...
	m_pGraphics->m_Entities.Add((CObject*)pMText);
	pMText->m_nLayer=layerIndex;
	pMText->m_Text="Static Water Level:";//"稳定水位（埋深/标高）：";
	pMText->m_Height=m_FontHeight;
	pMText->m_Align=AD_MTEXT_ATTACH_TOPLEFT;
	pMText->m_Location.x=left+112-subNum;//left+227;
	pMText->m_Location.y=top-28;//-18;
	strcpy(pMText->m_Font,ENFONTNAME);
...
//	CADMText* pMText;
	...
	m_pGraphics->m_Entities.Add((CObject*)pMText);
	pMText->m_nLayer=layerIndex;
	pMText->m_Text="Started Date:";//"开工日期：";
	pMText->m_Height=m_FontHeight;
	pMText->m_Align=AD_MTEXT_ATTACH_TOPRIGHT;
	pMText->m_Location.x=left+210-subNum;
	pMText->m_Location.y=top-8;
	strcpy(pMText->m_Font,ENFONTNAME);
...
//	CADMText* pMText;
	...
	m_pGraphics->m_Entities.Add((CObject*)pMText);
	pMText->m_nLayer=layerIndex;
	pMText->m_Text="Finished Date:";//"完工日期：";
	pMText->m_Height=m_FontHeight;
	pMText->m_Align=AD_MTEXT_ATTACH_TOPRIGHT;
	pMText->m_Location.x=left+210-subNum;
	pMText->m_Location.y=top-18;
...
//
	...
	pMText->m_Align=AD_MTEXT_ATTACH_TOPRIGHT;
	pMText->m_Location.x = left+GetGridColLeft(10)+m_GridColWidth[9]/2 + 1.8;
...
//============================================================================
	...
	m_pGraphics->m_Entities.Add((CObject*)pLine);
	pLine->m_nLayer=layerIndex;
	pLine->pt1.x=left+GetGridColLeft(11)+2;
	pLine->pt1.y=gridTop-17;
...
void CHistogramIniFile3::CreateChartFooter(double left,double top,double holeDeep)
{
	int i;
	CADLine* pLine;
	CADMText* pMText;
...
//	ADPOINT* pPoint;
	...
	m_pGraphics->m_Entities.Add((CObject*)pMText);
	pMText->m_nLayer=layerIndex;
	pMText->m_Text="Bore Profile";//"孔柱状图";
	pMText->m_Height=m_FontHeight-1;
	pMText->m_Align=AD_MTEXT_ATTACH_MIDDLERIGHT;
	pMText->m_Location.x=left+GetFootGridColLeft(3)-2;
	pMText->m_Location.y=footGridTop-m_FootGridHeight/2;
...
void CHistogramIniFile3::CreateChartFooter(LPCTSTR lpFilename)
{
...
void CHistogramIniFile3::CreateHatch(char* pLegendName,CADPolyline* pPolyline,int nLayer)
{
...
void CHistogramIniFile3::CreateHatchLine(char* hatchLineStr,CADHatch* pHatch)
{
...
void CHistogramIniFile3::CreateBeatNum(LPCTSTR lpFilename,double left,int num)
{
//原位测试击数-----------------------------------------------
	CADLine* pLine;
	CADMText* pMText;
	...
	GetPrivateProfileString(keyName,"1","",str,maxStrLen,lpFilename);
...
/*
//added on 20050909
	GetPrivateProfileString(keyName,"0","",str,maxStrLen,lpFilename);
	...
	//CreateBeatNum2(lpFilename,left,num);
}

void CHistogramIniFile3::CreateBeatNum2(LPCTSTR lpFilename,double left,int num)
{
	///CADLine* pLine;
	///CADMText* pMText;
	char str[2000];
...
void CHistogramIniFile3::ProcessHatch(CADPolyline* pPolyline)
{
...
double CHistogramIniFile3::GetFootGridColLeft(int i)
{
	double left;
	left=m_GridLeft;
	for(int j=0;j<i-1;j++)
