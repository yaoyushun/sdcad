#include "stdafx.h"
#include "HistogramIniFile2.h"
#define LegendWidth 0.8//unit cm
#include "..\FunctionLib\Function_System.h"

#define ENFONTNAME "Arial"

CHistogramIniFile2::CHistogramIniFile2(CADLayerGroup* pLayerGroup,CADGraphics* pGraphics)
{
...
//initialize===============================================
	...
	m_GridColWidth[curColIndex++]=10*7;//标准贯入试验曲线
//==========================================================
	...
	m_FootGridColWidth[curFootColIndex++]=48;//95
...
CHistogramIniFile2::~CHistogramIniFile2()
{
	delete[] m_GridColWidth;
	delete[] m_FootGridColWidth;
}

void CHistogramIniFile2::FileImport(LPCTSTR lpFilename)
{
...
double CHistogramIniFile2::GetGridColLeft(int i)
{
	double left;
	left=m_GridLeft;
	for(int j=0;j<i-1;j++)
...
CADBlock* CHistogramIniFile2::CreateLayerBlock(CPerHoleLayer* pHoleLayer)
{
...
void CHistogramIniFile2::CreateGrid()
{
	int i;
	CADLine* pLine;
	CADMText* pMText;
...
void CHistogramIniFile2::CreateChartHeader()
{
...
//----------------------------------------------------------------------
	...
	pMText->m_Location.y = 201.5 + 30 - 10;

}

void CHistogramIniFile2::CreateChartFrame()
{
	CADPolyline* pPolyline;
	ADPOINT* pPoint;
	pPolyline=new CADPolyline();
...
void CHistogramIniFile2::CreateChartGrid(double left,double top,double holeDeep)
{
//	return;
	int layerIndex = 0;
...
//	CADLine* pLine; |------------
	...
	m_pGraphics->m_Entities.Add((CObject*)pMText);
	pMText->m_nLayer=layerIndex;
	pMText->m_Text="Project Name:";//"工程名称：";
	pMText->m_Height=m_FontHeight;
	pMText->m_Align=AD_MTEXT_ATTACH_TOPRIGHT;
...
//	CADMText* pMText;
	...
	pMText->m_Height=m_FontHeight;
	pMText->m_Align=AD_MTEXT_ATTACH_TOPLEFT;
	pMText->m_Location.x=left+112;//left+227;
...
//含水量
	...
	pLine->pt1.y=gridTop-17;
	pLine->pt2.x=left+GetGridColLeft(9)+m_GridColWidth[8]-2;
	pLine->pt2.y=gridTop-17;

	pMText=new CADMText();
	m_pGraphics->m_Entities.Add((CObject*)pMText);
	pMText->m_nLayer=layerIndex;
	pMText->m_Text="From～To";//"孔 深";
	pMText->m_Height=m_FontHeight;
	pMText->m_Align=AD_MTEXT_ATTACH_TOPCENTER;
	pMText->m_Location.x=left+GetGridColLeft(9)+m_GridColWidth[8]/2;
	pMText->m_Location.y=gridTop-19;
	...
	m_pGraphics->m_Entities.Add((CObject*)pMText);
	pMText->m_nLayer=layerIndex;
	pMText->m_Text="histogram";//"SPT test graph";//"标准贯入试验曲线";
	pMText->m_Height=m_FontHeight;
	pMText->m_Align=AD_MTEXT_ATTACH_TOPCENTER;
...
void CHistogramIniFile2::CreateChartFooter(LPCTSTR lpFilename)
{
...
void CHistogramIniFile2::CreateHatch(char* pLegendName,CADPolyline* pPolyline,int nLayer)
{
...
void CHistogramIniFile2::CreateHatchLine(char* hatchLineStr,CADHatch* pHatch)
{
...
void CHistogramIniFile2::CreateBeatNum(LPCTSTR lpFilename)
{
//原位测试击数-----------------------------------------------
	CADLine* pLine;
	CADMText* pMText;
	...
	const short curCol = 9;

	while(pszTemp)
	{
		float rulerWidth = 0.0;
		rulerWidth = atof(pszTemp);
		...
		pSolid->m_pt0.x=GetGridColLeft(curCol+1);
		pSolid->m_pt0.y=sampleTop + 1.0;
		//TransPoint(&pSolid->m_pt0);
		pSolid->m_pt1.x=pSolid->m_pt0.x+rulerWidth;
		pSolid->m_pt1.y=pSolid->m_pt0.y;
		//TransPoint(&pSolid->m_pt1);
		pSolid->m_pt2.x=pSolid->m_pt0.x;
		pSolid->m_pt2.y=sampleTop-1.0;
		//TransPoint(&pSolid->m_pt2);
		pSolid->m_pt3.x=pSolid->m_pt1.x;
		pSolid->m_pt3.y=pSolid->m_pt2.y;
		//TransPoint(&pSolid->m_pt3);*/

		DrawRect(GetGridColLeft(curCol+1),sampleTop + 1.0,rulerWidth,2.0);

		/*pLine=new CADLine();	
		m_pGraphics->m_Entities.Add((CObject*)pLine);
		pLine->m_nLayer=curLayerIndex;
		pLine->pt1.x=GetGridColLeft(curCol+1);
		pLine->pt1.y=sampleTop;
		pLine->pt2.x=pLine->pt1.x+10.0;
		pLine->pt2.y=sampleTop;*/
	}
/*
//added on 20050909
...
void CHistogramIniFile2::CreateBeatNum2(LPCTSTR lpFilename)
{
//	CADLine* pLine;
//	CADMText* pMText;
	char str[2000];
...
void CHistogramIniFile2::ProcessHatch(CADPolyline* pPolyline)
{
...
double CHistogramIniFile2::GetFootGridColLeft(int i)
{
	double left;
	left=m_GridLeft;
	for(int j=0;j<i-1;j++)
...
void CHistogramIniFile2::DrawRect(double left,double top,double width,double height)
{
	CADPolyline* pPolyline;
	ADPOINT* pPoint;
	pPolyline=new CADPolyline();
